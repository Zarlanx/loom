...
proptest! {
    ...
    fn fencing_invariants_hold_under_arbitrary_interleavings(
        ...
        for idx in steps {
            ...
            let expected_no = AttemptNo(u32::try_from(seq).unwrap_or(u32::MAX));
            prop_assert_eq!(grant.attempt_no, expected_no);
            prop_assert_eq!(book.attempt_no(job), Some(expected_no));
...
fn make_node(
    ...
    Node {
        ...
        memory: MemoryModel::new(MemoryKind::Unified, size_mb),
        // Always advertise MLX so backend resolution never gates this test.
...
fn test_claim() -> ResourceClaim {
    ResourceClaim {
        ...
        supported_backends: BackendSet::from_backends(&[Backend::Mlx]),
...
proptest! {
    ...
    fn plan_placement_selects_the_argmax_feasible_node(
        specs in prop::collection::vec(
            (any::<bool>(), 0u64..80_000, 0i64..2_000, 0u16..=1_000, any::<bool>(), any::<bool>()),
            0..24,
        ),
    ) {
        ...
        let nodes: Vec<Node> = specs
            .iter()
            .enumerate()
            .map(|(i, &(available, size, price, rel, _, _))| {
                make_node(i, available, size, price, rel)
            })
            .collect();
        let offers: Vec<NodeOffer> = nodes
            .iter()
            .zip(specs.iter())
            .map(|(node, &(_, _, _, _, cache_warm, data_local))| NodeOffer {
                node,
