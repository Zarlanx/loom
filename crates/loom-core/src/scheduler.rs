...
mod tests {
    ...
    fn each_hard_constraint_can_exclude_a_node() {
        ...
        // CUDA floor: a claim demanding a CUDA line excludes a node that advertises no
        // CUDA at all and one whose CUDA is below the floor; a node meeting the floor
        // passes. The base node has `cuda: None`.
        let needs_cuda = ResourceClaim {
            min_cuda: Some(Version::new(12, 0, 0)),
            ..claim()
        };
        assert!(!is_feasible(&needs_cuda, &node("n"))); // node advertises no CUDA
