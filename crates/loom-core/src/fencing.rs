...
pub enum FenceError {
    ...
    /// A fenced write presented a fence that is not the lineage's current one — a
...
impl LeaseBook {
    ...
    ///
    /// # Errors
    /// Returns [`FenceError::UnknownLineage`] if the lineage does not exist, or
    /// [`FenceError::StaleFence`] if `fence` is not the lineage's current fence.
