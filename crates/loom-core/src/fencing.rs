...
impl LeaseBook {
    ...
    /// fence, so only the live attempt may retire it. A superseded attempt — one still
    /// holding a pre-requeue fence — cannot tear down a lineage that has already moved on to
    /// a greater fence. Because the global minter only ever increases, a `JobId` scheduled
