...
//! so [`JobEvent::Cancel`] is accepted from any non-terminal state. The same
//! prose-over-diagram reading applies to loss and preemption: the reconciliation
//! rules (control-plane §3.1) requeue a lost or stuck attempt and the preemption
//! rules (§3) eject an attempt the moment its node is claimed, so
//! [`JobEvent::Lose`]/[`JobEvent::Preempt`] are accepted from every live phase
//! (`scheduled`/`dispatched`/`preparing`/`running`/`checkpointing`), not only the
//! two loss edges the diagram happens to draw.
...
impl Default for ResourceClaim {
    /// A claim with no constraints — every field at its most permissive end, so any
    /// schedulable node satisfies it. This is **not** the derived `Default`: the
    /// derive would leave `supported_backends` empty (making [`BackendSelector::Auto`]
    /// resolve to nothing) and `max_price_per_sec_micro_usd` at `0` (the tightest
    /// possible ceiling), so a default claim could never be placed. The floors
    /// (`min_reliability_milli`, `min_isolation`) already default to their permissive
    /// ends; this brings the two ceiling-style fields into line.
    fn default() -> Self {
        Self {
            min_memory_mb: 0,
            gpu_model: None,
            min_driver: None,
            min_cuda: None,
            min_isolation: IsolationTier::default(),
            min_reliability_milli: 0,
            region_pref: None,
            // A price ceiling of `i64::MAX` filters out no node on price.
            max_price_per_sec_micro_usd: i64::MAX,
            backend: BackendSelector::default(),
            // Every backend, so `Auto` resolution is gated only by node capability.
            supported_backends: BackendSet::from_backends(&Backend::ALL),
        }
    }
}
...
impl JobState {
    ...
    pub fn apply(self, event: JobEvent) -> Result<Self, IllegalTransition> {
        ...
        let next = match (self, event) {
            ...
            // Loss (agent silence / lease lapse) and preemption (owner-eject /
            // higher class) reach any live phase, not just Running: a stuck
            // `preparing` attempt requeues (control-plane §3.1) and owner-eject
            // preempts a node the moment its lease commits (§3 preemption rules).
            (Self::Running, JobEvent::Lose) => Self::Requeued,
            (Self::Scheduled, JobEvent::Lose) => Self::Requeued,
            (Self::Dispatched, JobEvent::Lose) => Self::Requeued,
            (Self::Preparing, JobEvent::Lose) => Self::Requeued,
            (Self::Checkpointing, JobEvent::Lose) => Self::Requeued,
            (Self::Running, JobEvent::Preempt) => Self::Preempted,
            (Self::Scheduled, JobEvent::Preempt) => Self::Preempted,
            (Self::Dispatched, JobEvent::Preempt) => Self::Preempted,
            (Self::Preparing, JobEvent::Preempt) => Self::Preempted,
            // `Checkpointing` is the one live phase whose `Preempt` does not park in
            // `Preempted`: control-plane §3 draws it explicitly as
            // `checkpointing --> requeued: preempt mid-snapshot`. A snapshot cut short by
            // an eject cannot complete, so the attempt requeues from the last durable
            // checkpoint rather than await a `Preempted -> Requeue` step it can never run.
...
mod tests {
    ...
    const LEGAL: &[(JobState, JobEvent, JobState)] = &[
        ...
        (JobState::Scheduled, JobEvent::Lose, JobState::Requeued),
        (JobState::Dispatched, JobEvent::Lose, JobState::Requeued),
        (JobState::Preparing, JobEvent::Lose, JobState::Requeued),
        (JobState::Checkpointing, JobEvent::Lose, JobState::Requeued),
        (JobState::Running, JobEvent::Preempt, JobState::Preempted),
        (JobState::Scheduled, JobEvent::Preempt, JobState::Preempted),
        (JobState::Dispatched, JobEvent::Preempt, JobState::Preempted),
        (JobState::Preparing, JobEvent::Preempt, JobState::Preempted),
        (
            JobState::Checkpointing,
            JobEvent::Preempt,
            JobState::Requeued,
    ...
    fn every_state_event_pair_matches_the_hand_written_table() {
        ...
        // 21 non-cancel edges + one cancel edge from each of the 9 non-terminal states.
    ...
    #[test]
    fn default_claim_is_permissive_not_infeasible() {
        // The manual `Default` is the *no-constraints* claim, not the derived one: a
        // derived default would leave `supported_backends` empty (so `Auto` resolves to
        // nothing) and the price ceiling at `0` (the tightest possible), making a default
        // claim impossible to place. `scheduler` carries the end-to-end feasibility proof.
        let claim = ResourceClaim::default();
        assert_eq!(claim.min_memory_mb, 0);
        assert_eq!(claim.min_reliability_milli, 0);
        assert_eq!(claim.max_price_per_sec_micro_usd, i64::MAX);
        assert_eq!(claim.backend, BackendSelector::Auto);
        assert!(claim.min_cuda.is_none());
        assert!(claim.gpu_model.is_none());
        assert!(claim.region_pref.is_none());
        // Every backend is supported, so `Auto` is gated only by node capability.
        for backend in Backend::ALL {
            assert!(claim.supported_backends.contains(backend));
        }
    }
    ...
    #[test]
    fn a_lost_or_preempted_pre_running_attempt_escapes() {
        // Loss and preemption are legal from every live phase, not just Running,
        // so an agent that goes silent before `RunStarted` never strands the job.
        for live in [
            JobState::Scheduled,
            JobState::Dispatched,
            JobState::Preparing,
            ...
            assert!(
                live.apply(JobEvent::Lose).is_ok(),
                "{live:?} must be losable"
            );
        }
        assert_eq!(
            JobState::Dispatched.apply(JobEvent::Lose),
            Ok(JobState::Requeued)
        );
        assert_eq!(
            JobState::Preparing.apply(JobEvent::Preempt),
