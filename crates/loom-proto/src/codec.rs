...
///
/// On QUIC each stream is a real transport stream and the tag is omitted; on the WSS
/// fallback the tag rides in the frame header so a slow log upload cannot head-of-line
/// block a heartbeat.
...
mod tests {
    use super::{
        Channel, CodecError, MAX_FRAME_LEN, decode_frame, decode_message, decode_wss_frame, frame,
        wss_frame,
    };
