...
impl GoldenVector {
    /// The checked-in filename for this vector, e.g. `envelope_header.bin`.
    ///
    /// The single owner of the `<name>.bin` convention: the regen tool and the
    /// golden test both derive filenames through here so they cannot drift.
    #[must_use]
...
pub fn vectors() -> Vec<GoldenVector> {
    ...
    vec![
        // The body-less header vector, fully populated per the module contract:
        // `correlation_id` is non-empty so this vector exercises that field's tag and
        // encoding, which proto3 would otherwise omit from the wire when it is default.
        vector(
            "envelope_header",
            &Envelope {
                correlation_id: ulid("ENVCORR"),
                ..header("ENVHDR")
            },
        ),
        vector(
        ...
        // A discrete-GPU enrollment: covers the `HardwareInventory` fields that are
        // proto3-default on the unified-memory node above (`vram_mb`, `cuda_version`,
        // `pcie_gen`, `ecc`, `power_cap_w`, `thermal_cap_c`), so a tag renumber on any of
        // them changes at least one golden.
        vector(
            "enroll_request_discrete",
            &envelope(
                "ENROLLREQNV",
                "",
                Body::EnrollRequest(enroll_request_discrete()),
            ),
        ),
        vector(
            "enroll_grant",
        ...
        // The synthetic drop-marker chunk: covers `dropped_marker`/`dropped_count`, which
        // are proto3-default on the normal chunk above.
        vector(
            "log_chunk_dropped",
            &envelope("LOGCHUNKDROP", "", Body::LogChunk(log_chunk_dropped())),
        ),
    ]
}
...
/// A discrete-GPU enrollment, coherently populating the `HardwareInventory` fields that
/// are proto3-default on the unified-memory [`enroll_request`] node (a datacenter NVIDIA
/// card reports discrete VRAM, a CUDA version, a `PCIe` generation, ECC, and power/thermal
/// caps). `unified_memory_mb` is now `0` — the unified node above keeps that tag covered.
fn enroll_request_discrete() -> EnrollRequest {
    EnrollRequest {
        enroll_token: "enrol-token-discrete-node".to_string(),
        hw: Some(HardwareInventory {
            gpu_model: "NVIDIA H100 80GB HBM3".to_string(),
            vram_mb: 81_920,
            ...
            power_cap_w: 700,
            thermal_cap_c: 87,
            backends: vec![Backend::Cuda as i32, Backend::Cpu as i32],
            memory_model: MemoryModel::DiscreteVram as i32,
            unified_memory_mb: 0,
            backend_runtimes: vec![BackendRuntime {
...
/// The synthetic drop-marker chunk (`dropped_marker = true`) a node emits after a
/// drop-oldest overflow. It carries no payload data; its job is to report `dropped_count`,
/// so this vector is what keeps the `dropped_marker`/`dropped_count` tags on the wire.
fn log_chunk_dropped() -> LogChunk {
    LogChunk {
        attempt_id: attempt_id(),
...
mod tests {
    use crate::v1::{
        AgentState, Backend, ExitClass, LogStream, MemoryModel, NatClass, RejectReason,
    };

    /// Every M1 enum discriminant is frozen to its exact wire number (agent-protocol.md §2.3).
    ///
    /// The golden vectors above encode only *selected* enum values — for example, only
    /// `Backend::{Mlx,Cpu,Cuda}`, `AgentState::Running`, `NatClass::Cone`,
    /// `RejectReason::Thermal`, and `ExitClass::NodeFault`. Renumbering an *uncovered*
    /// value such as `Backend::Rocm` would leave every checked-in `.bin` byte unchanged
    /// and slip past the golden-vector gate. This test pins every declared variant of every
    /// M1 enum — including the proto3 zero/default `Unspecified` values — to its intended
    /// discriminant, so any renumber fails here independently of golden coverage. The
    /// assertion count is exhaustive: adding a variant without a line here does not weaken
    /// the freeze (the new variant is simply untested until listed), but *renumbering* an
    /// existing one always trips a listed assertion.
    #[test]
    fn m1_enum_discriminants_are_frozen() {
        // Backend (common.proto) — ADR-0015 priority order MLX → CUDA → CPU, ROCm last.
        assert_eq!(Backend::Unspecified as i32, 0);
        assert_eq!(Backend::Mlx as i32, 1);
        assert_eq!(Backend::Cuda as i32, 2);
        assert_eq!(Backend::Cpu as i32, 3);
        assert_eq!(Backend::Rocm as i32, 4);

        // MemoryModel (common.proto).
        assert_eq!(MemoryModel::Unspecified as i32, 0);
        assert_eq!(MemoryModel::Unified as i32, 1);
        assert_eq!(MemoryModel::DiscreteVram as i32, 2);

        // NatClass (enrollment.proto).
        assert_eq!(NatClass::Unspecified as i32, 0);
        assert_eq!(NatClass::Open as i32, 1);
        assert_eq!(NatClass::Cone as i32, 2);
        assert_eq!(NatClass::Symmetric as i32, 3);

        // AgentState (health.proto).
        assert_eq!(AgentState::Unspecified as i32, 0);
        assert_eq!(AgentState::Enrolling as i32, 1);
        assert_eq!(AgentState::Idle as i32, 2);
        assert_eq!(AgentState::Claimed as i32, 3);
        assert_eq!(AgentState::Preparing as i32, 4);
        assert_eq!(AgentState::Running as i32, 5);
        assert_eq!(AgentState::TearingDown as i32, 6);
        assert_eq!(AgentState::Draining as i32, 7);

        // RejectReason (job.proto) — LOCAL_POLICY is the proto3 default (0).
        assert_eq!(RejectReason::LocalPolicy as i32, 0);
        assert_eq!(RejectReason::Thermal as i32, 1);
        assert_eq!(RejectReason::OwnerWindow as i32, 2);
        assert_eq!(RejectReason::Resource as i32, 3);
        assert_eq!(RejectReason::ManifestSig as i32, 4);
        assert_eq!(RejectReason::TierUnsupported as i32, 5);

        // ExitClass (job.proto) — UNSPECIFIED is the neutral proto3 default (0).
        assert_eq!(ExitClass::Unspecified as i32, 0);
        assert_eq!(ExitClass::Oom as i32, 1);
        assert_eq!(ExitClass::NonzeroExit as i32, 2);
        assert_eq!(ExitClass::SandboxViolation as i32, 3);
        assert_eq!(ExitClass::NodeFault as i32, 4);

        // LogStream (log.proto).
        assert_eq!(LogStream::Unspecified as i32, 0);
        assert_eq!(LogStream::Stdout as i32, 1);
        assert_eq!(LogStream::Stderr as i32, 2);
        assert_eq!(LogStream::Event as i32, 3);
    }
}
