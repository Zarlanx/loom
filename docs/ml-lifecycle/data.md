...
- **Retention / deletion API** — `loom data rm <manifest>` deletes the renter's chunks from Loom object store and issues cache-invalidation to any node holding them; content-addressing means a chunk shared with another live dataset is refcounted, not orphaned. Deletion is auditable. **Deletion is authoritative on infrastructure the operator/user controls** (the object store, and any node under your control — always true in the self-host profiles) but **best-effort on untrusted marketplace hosts**: cache-invalidation is a request a *malicious* host can ignore, and a host that copied chunks out of its cache can retain them regardless. This is the same consumer-hardware honesty as the rest of this section — a stranger's node is untrusted, so deletion from it cannot be *guaranteed*, only requested and audited (see [security.md](../platform/security.md)). Public/synthetic/licensed data on the consumer tier is the case where this is acceptable; sensitive data should not be there in the first place.
