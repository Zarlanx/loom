...
**Auth (sealed secrets).** A user's HF token is stored as a **sealed secret** — encrypted, injected into the job's environment at runtime, never written to disk in plaintext, never logged, never surfaced in job manifests or heartbeats. This keeps the token off the host's disk and logs. **Honesty caveat for Tier A/B:** sealing does **not** make the token invisible to a *live* malicious host — on consumer cards a host owner who actively dumps guest RAM can read any secret resident in memory, exactly as they can read weights and activations ([security.md §3.1](../platform/security.md)). Sealed secrets are hygiene (off-disk, off-logs), not a confidentiality guarantee against an active host; only Tier C changes that. **Accepted improvement — token brokering:** for gated HF models the platform can **broker the download through the control/artifact service** so the raw HF token never enters the untrusted sandbox at all — Loom fetches the gated weights control-plane-side and places them on the node via the normal content-addressed cache, and the job receives weights, not a credential. This removes the token from the untrusted node's reach entirely for the common gated-model case. **Flag:** the exact sealing/attestation and brokering mechanism is a platform-security detail to be specified alongside [host-agent.md](../platform/host-agent.md).
