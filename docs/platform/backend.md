...
| QUIC | **quinn** | Async QUIC on tokio + rustls; the agent-gateway's primary transport (agent-protocol §1.1). Production-quality, founded 2018, 30+ releases, pluggable crypto (rustls/aws-lc-rs incl. FIPS).[^quinn] |
| WSS fallback | **tokio-tungstenite** | The portable WSS-over-TCP/443 fallback terminator (agent-protocol §1.1). Best-maintained Rust WebSocket lib, rustls TLS via feature. |
...
**The outbox pattern replaces NATS durability in embedded mode.** In standalone there is no JetStream to provide durable, replayable event streams. The `outbox` table is the substitute: a state change and the event announcing it are written in **one transaction** (row change + `outbox` row). The `loom-bus` outbox relay task then publishes unsent `outbox` rows to the active bus (`InProcBus` fan-out to in-process subscribers) and marks them sent. This gives at-least-once delivery with the *same* dedupe-on-event-ID discipline the marketplace uses with JetStream (control-plane §3) — so business logic is identical across profiles. `usage_records` remain idempotent via the `(attempt_id, seq)` UNIQUE constraint, making at-least-once replay exactly-once for billing on either bus.
...
- **No streaming replica / PITR at the same maturity.** The crown-jewels backup story (WAL archiving, PITR, read replicas) is a Postgres strength; SQLite backup is file-copy/`VACUUM INTO`, adequate for one box, not for a fleet.
...
[agent_gateway]
enabled  = true
quic_bind = "127.0.0.1:8444"            # loom-hostd connects here; standalone binds localhost. In fleet mode agents reach the coordinator over the API port (self-host §3, "one open port"); this internal QUIC listener is not a second exposed port.
