...
- **`loom-sandbox`** — runtime drivers behind a `SandboxDriver` trait: `RuncDriver`/`YoukiDriver` (Tier B), `CloudHypervisorDriver` (Tier A), and `ProcessDriver` — the macOS host-process driver that runs a job as a plain child process with **no isolation** (trusted-profile only; the only path to Metal / the MLX backend on Apple silicon, [../architecture/profiles.md](../architecture/profiles.md), [ADR-0015](../adr/0015-pluggable-compute-backends.md)). Execution driver and compute backend are orthogonal axes (compute-backends.md): the driver decides how a job is contained, the backend decides what it computes on. This crate is a dependency of `loom-hostd`, **not** `loomd` — the server never drives a sandbox. It is listed in the workspace because the CLI's local `loom dev` mode (standalone-on-one-box) may co-launch `loom-hostd`, and because the driver trait's fakes are shared test infrastructure.
...
2. **Runs filter → score → commit** per schedulable unit (control-plane §4). Filter is a `WHERE` against the nodes index — including the node's advertised **backend capability** (`mlx`/`cuda`/`cpu`/`rocm`), so a CUDA job never lands on an MLX-only node and vice versa ([compute-backends.md](./compute-backends.md), [ADR-0015](../adr/0015-pluggable-compute-backends.md)); score is pure `loom-core` logic; commit writes a `lease` row guarded by `INSERT ... ON CONFLICT` against the node's current status.
...
**Migration rollback rules.** Binary rollback without schema compatibility is not rollback, so upgrades obey an **N−1 schema-compatibility contract**: every migration shipped in release N must leave a database that binary N−1 can still run against (additive changes land freely; destructive changes — drops, renames, type narrowing — are split across two releases: N adds-and-dual-writes, N+1 removes). `loomd upgrade` takes an automatic pre-upgrade snapshot (`VACUUM INTO` on SQLite; `pg_dump` on Postgres) *before* applying migrations, and the staged-upgrade auto-rollback ([self-host.md](../product/self-host.md) §6) is armed only while the N−1 contract holds — if a release ever has to break it, the upgrade is flagged irreversible up front and rollback means restoring the snapshot. Migrations are forward-only (no down-migrations to maintain and mistrust); the snapshot is the down path.
...
- **Simulated-fleet integration.** The headline test: an in-process `loomd` (SQLite `Store`, `InProcBus`, embedded agent-gateway; the chaos configuration runs on a **file-backed WAL store** so crash/restart recovery is exercised against real persistence, with an in-memory variant kept only for fast inner-loop runs) driven by **N fake agents** with **chaos** — random disconnects, owner-ejects, silence past 90 s, stale-fence late writes. Assertions: every lost attempt requeues with a strictly greater fence; no attempt is billed twice; no two agents ever hold an accepted lease on the same attempt (the split-brain invariant, agent-protocol §5). Because every service is a library (§1), this whole scenario runs in one process with no sockets and no root.
