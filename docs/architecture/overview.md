...
Loom is a self-hostable GPU compute stack for the full ML lifecycle. The same components run in three [deployment profiles](./profiles.md): **standalone**, collapsed onto a single GPU box you own; **private fleet**, with the control plane coordinating your own machines as hosts; and **marketplace**, the deferred third profile that opens the fleet to strangers renting idle GPUs. This document describes the components once; the profiles doc explains how they collapse or spread. In every profile a host runs a lightweight agent, and renters run ML workloads — testing, training, fine-tuning — in managed sandboxes, or serve models behind an OpenAI-compatible inference API. The defining constraint of the marketplace profile is that supply lives on *other people's machines* — residential connections behind NAT, daily-driver desktops that also play games, headless rigs in a closet — and the architecture is built to tolerate that; the self-host profiles inherit the same resilience on hardware you control.

This is the top-level map. It defines the actors, names every component, walks two end-to-end flows, records the technology bets, and — just as importantly — states plainly what Loom is *not*. Sibling documents go deep on each subsystem: the [deployment profiles](./profiles.md), the [host agent](../platform/host-agent.md), [isolation](../platform/isolation.md), the [control plane](../platform/control-plane.md), [networking](../platform/networking.md), and [security](../platform/security.md); and on the ML side, [data](../ml-lifecycle/data.md), [training](../ml-lifecycle/training.md), [evaluation](../ml-lifecycle/evaluation.md), [serving](../ml-lifecycle/serving.md), and [environments](../ml-lifecycle/environments.md).
...
**Operators** run the control plane, gateways, and — in the fleet/marketplace profiles — relay infrastructure. In the standalone and private-fleet [profiles](./profiles.md) the operator is *you*, self-hosting on your own hardware, and the operator role collapses into a single lightweight process; in the deferred marketplace profile the operator is the Loom team. Operator infrastructure is deliberately small and boring in every profile — it is a coordination layer, not a datacenter. Operators also curate the runtime image catalog and set trust-tier labeling policy; when the marketplace profile is active, they additionally adjudicate the [marketplace](../product/marketplace.md) (pricing floors, payouts, dispute handling) — machinery that is absent when you self-host.
...
**Control plane.** API + scheduler + state, running on operator infrastructure. Technology is deliberately unexciting: a stateless API service, a scheduler, **Postgres** as the source of truth (jobs, users, nodes, billing ledger), and **NATS** as the message bus between control plane and the host fleet. In the self-host profiles this whole plane is realized as the single **`loomd`** binary with embedded SQLite and an in-process queue behind repository/bus traits; Postgres and NATS are the marketplace-scale configuration, not a core requirement ([backend.md](../platform/backend.md), [ADR-0013](../adr/0013-single-binary-self-host-control-plane.md)). A job spec is `image + resource claim + isolation tier + max price`; billing is per-second. Nodes are cattle — checkpoint, requeue, and failover are core paths, not afterthoughts, because a consumer host disappearing mid-job is the *expected* case. See [control-plane.md](../platform/control-plane.md).
...
| Control plane | SQLite embedded + in-process queue (core) / Postgres + NATS (marketplace scale) + custom scheduler | Kubernetes; Nomad; Ray | Hosts are single machines behind NAT, not a cluster we administer — K8s assumes a fleet you own and can reach inbound; we need a coordinator for cattle nodes, not an orchestrator (see below) |
